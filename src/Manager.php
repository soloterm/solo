...
class Manager
{
    ...
    protected function blockIfUnsafeTestCommand(Command $command): void
    {
        ...
        if ($this->usesTestingEnvironment($command)) {
            return;
        }
    ...
    protected function usesTestingEnvironment(Command $command): bool
    {
        if (($command->getEnvironment()['APP_ENV'] ?? null) === 'testing') {
            return true;
        }

        return (bool) preg_match('/(^|\s)APP_ENV=testing(\s|$)/', $command->command ?? '');
    }
